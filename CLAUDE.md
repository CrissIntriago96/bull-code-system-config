...
Repositorio de configuración (backend git de Spring Cloud Config) para los servicios de `bull-code-system-backend`: `rrhh-backend`, `api-gateway` y `notification-service`. Contiene los YAML y el pipeline de Jenkins que los aplica en producción (`Jenkinsfile` + `jenkins/*.sh`). Sigue la convención de `taurus-config` (otro repo del mismo autor), pero sin replicar sus secretos en texto plano. Todo el repo (comentarios, docs) está en español.
...
- **No despliega código.** Recrea con la misma imagen (`up --no-deps --no-build --force-recreate`) los contenedores que levantaron los jobs de `bull-code-system-backend`, para que relean su config. `--no-deps` es obligatorio: sin él, reiniciar el backend recrearía su postgres.
- **`jenkins/common.sh` (`service_info`) duplica datos del backend:** carpeta del stack, servicio en compose, puerto y smoke test de cada app. Tienen que coincidir con el `Jenkinsfile` y el `docker-compose.prod.yml` de cada servicio allá. Si cambian allá (un puerto, por ejemplo), cambian acá. `APPS` define también el orden de reinicio.
...
- Gateway: prefijo nuevo `spring.cloud.gateway.server.webflux.*` (el viejo `spring.cloud.gateway.*` está deprecado y no se aplica). La ruta comodín `/api/** → rrhh-backend` tiene `order: 1000`, así que un servicio nuevo suma su ruta en `api-gateway.yml` con un order menor y `uri: lb://<spring.application.name>`. El discovery locator queda apagado: toda ruta es explícita.
